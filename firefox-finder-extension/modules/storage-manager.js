...
export class StorageManager {
  ...
  async getLastSearch() {
    try {
      const result = await browserAPI.storage.local.get(this.storageKeys.LAST_SEARCH);
  ...
  async getSettings() {
    try {
      const result = await browserAPI.storage.local.get(this.storageKeys.USER_SETTINGS);
  ...
  async getCustomSystemPrompt() {
    try {
      const result = await browserAPI.storage.local.get(this.storageKeys.CUSTOM_SYSTEM_PROMPT);
