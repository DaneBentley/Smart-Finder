...
export class AIService {
  ...
  async checkRateLimit(estimatedApiCalls = 1) {
    ...
    const stored = await browserAPI.storage.local.get(['aiApiCallHistory']);
  ...
  async recordRequest(apiCalls = 1) {
    ...
    // Save to storage
    await browserAPI.storage.local.set({ aiApiCallHistory: this.apiCallHistory });
  }
  ...
  async searchBatch(query, batchContent, batchIndex) {
    try {
      // Get JWT token for authenticated request
      const tokenData = await browserAPI.storage.local.get(['jwt']);
