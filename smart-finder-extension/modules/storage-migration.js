...
export class StorageMigration {
  ...
  async checkAndMigrate() {
    try {
      const { migrationCompleted, jwt } = await chrome.storage.local.get(['migrationCompleted', 'jwt']);
      ...
      await chrome.storage.local.set({ migrationCompleted: this.migrationVersion });
      return { success: true, migrated: false };
  ...
  async cleanupLegacyStorage() {
    try {
      ...
      if (suspiciousKeys.length > 0) {
        await chrome.storage.local.remove(suspiciousKeys);
      }
  ...
  async forceMigration() {
    try {
      await chrome.storage.local.remove(['migrationCompleted']);
